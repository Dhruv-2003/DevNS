...
contract Domains is ERC721URIStorage {
    ...
    // mapping a  domain to the address
    mapping(string => address) public domains;

    mapping(uint256 => string) public names;

    //records mapping , so that the domian maps to some record
    ...
    function setRecord(string calldata name, string calldata record) public {
        // check the owener is the transaction sender or intiator
    ...
    // set record function
    function setTwitter(string calldata name, string calldata _twitter) public {
        // check the owener is the transaction sender or intiator
        require(domains[name] == msg.sender);
